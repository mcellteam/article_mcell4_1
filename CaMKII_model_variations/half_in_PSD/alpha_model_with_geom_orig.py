...
PSD_SC = m.SurfaceClass(
    name = 'PSD',
    properties = [reflect, Molec__Ca___Orient__Ignore___Type_Transparent, Molec__PP1___Orient__Ignore___Type_Transparent]
    #properties = [reflect]
)
model.add_surface_class(PSD_SC)
PSD.surface_class = PSD_SC
model.add_geometry_object(PSD)
...
if species_file:
    with open(INITIAL_SEED_SPECIES_FILE, 'r') as f_species:
        ...
        for line in f_species:
            ...
            if 'CaMKII' in name:
                rel_site1 = m.ReleaseSite(
                    name='Release of ' + name + 'in psd',
                    complex=m.Complex(name),
                    region=(PSD*spine), 
                    number_to_release=int(count/2)+int(count%2)
                )
                model.add_release_site(rel_site1)                
                rel_site = m.ReleaseSite(
                    name='Release of ' + name,
                    complex=m.Complex(name),
                    region=spine-PSD, 
                    number_to_release=int(count/2)
                )
            else:
                rel_site = m.ReleaseSite(
                    name='Release of ' + name,
